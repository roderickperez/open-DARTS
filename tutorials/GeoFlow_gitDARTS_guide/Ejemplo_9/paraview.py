...
for i in range(start_idx, end_idx + 1):
    ...
    # Extraer la variable presión del PointData
    ...
    # Extraer valores de presión en numpy array
    pressure_values = np.array([array.GetValue(j) for j in range(n_points)])
    ...
    presion_data.append(pressure_values)
...
for idx, pdata in enumerate(presion_data):

    if Select_case.case == '1':
        print("Simulando caso 1")
        df[f'{idx}0 dias'] = pdata
        # Guardar CSV
        output_csv = os.path.join(new_directory, "presion_sobre_linea_1.csv")
    
    elif Select_case.case == '2':
        print("Simulando caso 2")
        df[f'{idx} años'] = pdata
        # Guardar CSV
        output_csv = os.path.join(new_directory, "presion_sobre_linea_2.csv")
