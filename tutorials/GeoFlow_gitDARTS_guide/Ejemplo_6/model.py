...
class Model(DartsModel):
    ...
    def set_physics(self, n_points):
        ...
        property_container.relperm_ev = dict([('water', iapws_water_relperm_evaluator()) ,   
                                                 ('steam', iapws_steam_relperm_evaluator())]) 
        
        property_container.viscosity_ev = dict([('water', ConstFunc(0.02)) ,   #   [cP]
                                                 ('steam', ConstFunc(0.02))])  #   [cP]
        
        property_container.output_props = {'Kr_W': lambda: property_container.kr[0],
                                           'Kr_S': lambda: property_container.kr[1],
                                           'Visco_W': lambda: property_container.mu[0],
                                           'Visco_S': lambda: property_container.mu[1],
    ...
    def set_initial_conditions(self):
        ...
        self.input_distribution = {"pressure": 120.,           # [bar]
                            "enthalpy": 1398.5 * 18.01528   # [kJ/kg]-----[kJ/kmol]
                            # "pressure": 80.,           # [bar]
        ...
        state_init = value_vector([self.input_distribution['pressure'],self.input_distribution['enthalpy'] ])               # Initial presion   (bar)   

        Temp_init = self.physics.property_containers[0].temperature_ev.evaluate(state_init )
        
        estado=np.array([state_init[0],self.input_distribution['enthalpy'] ])
        ...
        print('Initial temperature (C)=',Temp_init-273.15)
        print('Initial enthalpy ([kJ/kg])=',self.input_distribution['enthalpy']/18.01528)
        print('Initial enthalpy ([kJ/kmol])=',self.input_distribution['enthalpy'])
    ...
    def set_wells(self):
        ...
        self.reservoir.add_perforation("INJ", cell_index=(1, 1, 1),  skin=0.0,   
                                           well_radius=0.1524, multi_segment=False, well_index=100)    # ,well_indexD=None
    ...
    def plot_init_phase_with_p_h(self, P_ini, h_ini, P_iny, T_iny, P_prod, h_prod):
        ...
        # === Punto Inicial: ahora usa P_ini y h_ini ===
        ...
        plt.text(h_punto/1000*.9, P_punto/1e5*.9, 
