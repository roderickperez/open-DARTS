...
class FluidFlowerStruct(StructReservoir):
    def __init__(self, timer, layer_properties, layers_to_regions, model_specs, well_centers):
        ...
        dx, dy, dz = x[1] - x[0], 0.025, z[1] - z[0]
    ...
    def set_wells(self, verbose: bool = False):
        for name, center in self.well_centers.items():
            ...
            self.add_well(name)
            self.add_perforation(name, res_cell_idx=cell_index, well_diameter=0.002, well_indexD=0,
                                 segment_direction='y_axis', verbose=verbose)

        self.add_well('P1')
        for n in range(self.nx):
            self.add_perforation("P1", res_cell_idx=(n + 1, 1, self.nz), well_index=100, well_indexD=0, verbose=verbose)
        return
