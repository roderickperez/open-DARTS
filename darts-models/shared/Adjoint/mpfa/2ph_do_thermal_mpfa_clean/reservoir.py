...
class UnstructReservoir:
    ...
    def ladder(self, discr_type, mesh_file):
        self.heat_cond = 2.1 * 86400.0 / 1000  # kJ/m/day/K
        self.perm_mat = 1000
        ...
        if discr_type == 'mpfa':
            ...
            dummy_perm = pp.perm * pp.anisotropy[0]
            # Construct instance of Unstructured Discretization class:
            self.unstr_dummy = UnstructDiscretizer(permx=dummy_perm,
                                                   permy=dummy_perm,
                                                   permz=dummy_perm,
                                                   frac_aper=frac_aper,
                                                   mesh_file=self.mesh_file)
            # Use class method load_mesh to load the GMSH file specified above:
            self.unstr_dummy.load_mesh()
            # Calculate cell information of each geometric element in the .msh file:
            self.unstr_dummy.calc_cell_information()
            self.unstr_dummy.store_centroid_all_cells()

            centroids = self.unstr_dummy.centroid_all_cells.copy()
            L = np.max(self.unstr_dummy.mesh_data.points)

            self.perm_base = np.diag((pp.perm * pp.anisotropy[0], pp.perm * pp.anisotropy[1], pp.perm * pp.anisotropy[2]))
            perm_list = []
            for i, cell_id in enumerate(range(centroids.shape[0])):
                c = centroids[cell_id]
                pp = self.porperm[0]
                if c[0] + c[1] <= L / 2:
                    r = R.from_rotvec([0, 0, np.pi / 4])
                    ...
                    r = R.from_rotvec([0, 0, np.pi / 4])
                perm = r.as_matrix().dot(self.perm_base).dot(r.as_matrix().T)
                # perm_list.append([perm[1, 1], perm[1, 1], perm[1, 1]])
                perm_list.append(np.diag(perm))

            perm_list = np.array(perm_list)

            # # perm_list[perm_list <= 10] = 2000
            # from matplotlib import pyplot as plt
            # plt.hist(perm_list)
            # plt.show()

            # Construct instance of Unstructured Discretization class:
            self.unstr_discr = UnstructDiscretizer(permx=perm_list[:, 0],
                                                   permy=perm_list[:, 1],
                                                   permz=perm_list[:, 2],
                                                   frac_aper=frac_aper,
                                                   mesh_file=self.mesh_file)
