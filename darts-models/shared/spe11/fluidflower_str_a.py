...
class FluidFlowerStruct(StructReservoir):
    ...
    def set_wells(self, verbose: bool = False):
        for name, center in self.well_centers.items():
            ...
            self.add_well(name, wellbore_diameter=0.0018)
            self.add_perforation(name, cell_index, well_radius=0.0009, well_indexD=0, segment_direction='y_axis', verbose=verbose)
