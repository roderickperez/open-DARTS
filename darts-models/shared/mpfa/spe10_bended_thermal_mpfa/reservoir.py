...
class UnstructReservoir:
    def __init__(self, discr_type, model_folder, n_vars):
        """
        ...
        self.spe10(discr_type, model_folder)
        if discr_type == 'mpfa':
            ...
            self.depth[:] = np.abs(self.depth_all_cells)
    ...
    def spe10(self, discr_type, model_folder):
        self.diff_coef = 0.0#8.64e-6
        frac_aper = 1.E-4

        # figure out nx, ny, nz
        self.nx, self.ny, self.nz = int(model_folder.split('_')[-3]), \
                                    int(model_folder.split('_')[-2]), \
                                    int(model_folder.split('_')[-1])


        self.porosity = np.flip(np.swapaxes(load_single_keyword(model_folder + '/poro.txt', 'PORO', cache=0).
                                    reshape(self.nz, self.ny, self.nx), 0, 2), axis=2).flatten()
        self.permeability = np.flip(np.swapaxes(load_single_keyword(model_folder + '/perm.txt', 'PERM', cache=0).
                                        reshape(self.nz, self.ny, self.nx, 3), 0, 2), axis=2).flatten()

        if "data_40_80_80" in model_folder:
            perm_min = 1.0
            self.permeability[self.permeability <= perm_min] = perm_min

        if discr_type == 'mpfa':
            self.mesh_file = model_folder + '/spe10_bended.msh'
            ...
            self.discr.grav_vec = matrix([0.0, 0.0, -9.80665e-5], 1, 3)  # 0.0??
            ...
            self.n_fracs = 0 #self.n_fracs = self.discr_mesh.region_ranges[elem_loc.FRACTURE][1] - self.discr_mesh.region_ranges[elem_loc.FRACTURE][0]
            self.n_bounds = self.discr_mesh.region_ranges[elem_loc.BOUNDARY][1] - self.discr_mesh.region_ranges[elem_loc.BOUNDARY][0]

            # read & set parameters
            assert (self.n_matrix == self.nx * self.ny * self.nz)

            # self.p_init = np.swapaxes(load_single_keyword(model_folder + '/pres.txt', 'PRESSURE', cache=0).
            #                                     reshape(self.nz, self.ny, self.nx), 0, 2).flatten()
            # self.ref_pressure = np.swapaxes(load_single_keyword(model_folder + '/ref_pres.txt', 'REF_PRESSURE', cache=0).
            #                                     reshape(self.nz, self.ny, self.nx), 0, 2).flatten()
            volumes = np.array(self.discr_mesh.volumes, copy=False)
            centroids = np.array(self.discr_mesh.centroids)
            self.volume_all_cells = np.copy(volumes[:self.n_matrix])
            ...
            for i, cell_id in enumerate(range(self.discr_mesh.region_ranges[elem_loc.MATRIX][0], self.discr_mesh.region_ranges[elem_loc.MATRIX][1])):
                ...
                self.discr.perms.append(matrix33(self.permeability[3 * cell_id],
                                                 self.permeability[3 * cell_id + 1],
                                                 self.permeability[3 * cell_id + 2]))
                self.discr.heat_conductions.append(matrix33(self.heat_cond,
                                                            self.heat_cond,
                                                            self.heat_cond))
            ...
            self.mesh_file = model_folder + '/spe10-tpfa_bended.msh'
            self.mesh_data = meshio.read(self.mesh_file)
            n_cells = np.size(self.mesh_data.cell_data["gmsh:geometrical"][0])

            permx = []
            permy = []
            permz = []
            for i, cell_id in enumerate(range(n_cells)):
                permx.append(self.permeability[3 * cell_id])
                permy.append(self.permeability[3 * cell_id + 1])
                permz.append(self.permeability[3 * cell_id + 2])

            # Construct instance of Unstructured Discretization class:
            self.unstr_discr = UnstructDiscretizer(permx=permx,
                                                   permy=permy,
                                                   permz=permz,
                                                   frac_aper=frac_aper,
    ...
    def write_to_vtk_old_discretizer(self, output_directory, cell_property, ith_step, engine):
        """
        ...
        for ith_geometry in self.unstr_discr.mesh_data.cells_dict.keys():
            if ith_geometry in available_matrix_geometries:
                ...
                #     if 'cell_id' not in cell_data: cell_data['cell_id'] = []
                    if 'depth' not in cell_data: cell_data['depth'] = []
                    if 'volume' not in cell_data: cell_data['volume'] = []
                    cell_data['perm'].append(np.zeros((self.unstr_discr.mat_cells_tot, 3), dtype=np.float64))
                    cell_data['depth'].append(np.zeros(self.unstr_discr.mat_cells_tot, dtype=np.float64))
                    cell_data['volume'].append(np.zeros(self.unstr_discr.mat_cells_tot, dtype=np.float64))
                    ...
                    cell_data['depth'][-1] = np.array(self.mesh.depth[:self.unstr_discr.mat_cells_tot])
                    cell_data['volume'][-1] = np.array(self.mesh.volume[:self.unstr_discr.mat_cells_tot])
        ...
        for cell_block in self.mesh_data.cells:
            if cell_block.type in available_matrix_geometries:
                ...
                if ith_step == 0:
                    ...
                    cell_data['cell_id'].append(np.zeros(len(cell_ids), dtype=np.int64))
                    cell_data['depth'].append(np.zeros(len(cell_ids), dtype=np.float64))
                    cell_data['volume'].append(np.zeros(len(cell_ids), dtype=np.float64))
                    for i, cell_id in enumerate(cell_ids):
                        cell_data['perm'][-1][i] = np.array(self.discr.perms[cell_id].values)
                        cell_data['cell_id'][-1][i] = cell_id
                        cell_data['depth'][-1][i] = np.array(self.mesh.depth[cell_id])
                        cell_data['volume'][-1][i] = np.array(self.mesh.volume[cell_id])
