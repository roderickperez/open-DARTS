...
class StructDiscretizer:
    ...
    def __init__(self, nx: int, ny: int, nz: int, global_data: dict, global_to_local=0, coord=0, zcorn=0, is_cpg=False):
        ...
        # If scalar dx, dy, and dz are specified: Store constant control volume dimensions
            ...
            self.centroids_all_cells[:, :, 1:, 2] = self.centroids_all_cells[:, :, 0, 2] + (d_cumsum[:, :, :-1] + d_cumsum[:, :, 1:]) * 0.5
            ...
            self.centroids_all_cells[:, 1:, :, 1] = self.centroids_all_cells[:, 0, :, 1] + (d_cumsum[:, :-1, :] + d_cumsum[:, 1:, :]) * 0.5
            ...
            self.centroids_all_cells[1:, :, :, 0] = self.centroids_all_cells[0, :, :, 0] + (d_cumsum[:-1, :, :] + d_cumsum[1:, :, :]) * 0.5
