...
/* ---------------------------------------------------------------------- */
{
    ...
    size_t        nx, ny, nz, c;
    size_t        origin, imax, jmax, id_shift, i_shift, j_shift;
    size_t        off[8];
	...
	id_shift = 0;
	do {
		i_shift = id_shift % (nx + 1);
		j_shift = (size_t)(id_shift / (nx + 1));
		origin = id_shift * (2 * 3);
		imax = (nx + (nx + 1) * j_shift) * (2 * 3);
		jmax = (i_shift + (nx + 1) * (ny + 0)) * (2 * 3);
