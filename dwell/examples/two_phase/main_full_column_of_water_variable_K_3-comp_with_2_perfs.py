...
Scenario description:
    ...
    Wellbore is perforated at multiple segments of the reservoir
"""
...
sources_sinks["Perforation_1"] = Perforation(pipe_name, segment_index, start_time, stop_time, pipe_geom, fluid_model,
...
# Perforation 2
pipe_name = "Well1"
segment_index = 2   # 0 is the index of the first segment
start_time = 0 * second()
stop_time = "end"
reservoir_pressure = well_initial_conditions.p_init_segments[segment_index] - 1
reservoir_fluid_mole_fractions = [1e-5, 1e-5, 1 - 2 * 1e-5]
...
skin = 0
perforation_hole_diameter = 2 * centi() * meter()   # Perforation hole diameter is needed for the kinetic energy and momentum calculations
sources_sinks["Perforation_2"] = Perforation(pipe_name, segment_index, start_time, stop_time, pipe_geom, fluid_model,
                                           reservoir_pressure, reservoir_fluid_mole_fractions, reservoir_cell_perm_list,
                                           reservoir_cell_size_list, perforation_hole_diameter, skin=skin, verbose=verbose)
...
#%% Time stepping
dt = 2 * second() * np.ones(900)   # 30 minutes
