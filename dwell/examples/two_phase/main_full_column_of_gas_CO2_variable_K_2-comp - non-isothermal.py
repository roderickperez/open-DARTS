"""
Injection of CO2 into a column of CO2 non-isothermally

Scenario description:
    Non-isothermal miscible 2-phase fluid flow with variable K values
    Impure CO2 injection into a wellbore full of gaseous CO2
...
Notes:
    This scenario requires small time steps at the beginning to converge, so generate_rampup_time_steps can help a lot.
    I have to use conditions that do not lead to liquid CO2 formation in the wellbore because if liquid CO2 forms, solution diverges.
"""
...
pipe_name = "Well1"
segments_lengths = 50 * meter() * np.ones(40)
pipe_ID = 0.1 * meter()
...
start_time = 0 * second()
stop_time = "end"
injection_mass_rate = 5 * kg() / second()

flow_direction = ["inflow", "along the pipe"]

injected_fluid_pressure = 20 * bar()
injected_fluid_temperature = (20 + 273.15) * Kelvin()
injected_fluid_mole_fractions = [0.999, 0.001]

injected_fluid_specific_enthalpy = enthalpy_eval['gas'].evaluate(injected_fluid_pressure, injected_fluid_temperature,
...
#%% Time stepping
dt = generate_rampup_time_steps(20, 10, 600)
