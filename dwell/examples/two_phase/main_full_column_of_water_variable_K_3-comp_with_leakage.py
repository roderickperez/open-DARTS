...
Scenario description:
    ...
    A leakage point is defined in a specific segment of the pipe.
"""
...
initial_C1_mole_fraction = [initial_fluid_conditions['phases_compositions'][0][1]] * pipe_geom.num_segments
...
injected_fluid_mole_fractions = [0.90, 0.05, 0.05]   # Impure CO2 injection

flow_direction = ["inflow", "along the pipe"]

# injection_specific_enthalpy = -20000 * Joule() / kg()  # Constant injection specific enthalpy
sources_sinks["ConstantMassRateSource_1"] = ConstantMassRateSource(pipe_name, pipe_geom, fluid_model, segment_index, start_time,
                                                                 stop_time, flow_direction, injection_mass_rate, isothermal,
                                                                 injected_fluid_mole_fractions, verbose=verbose)

# Leakage point
pipe_name = "Well1"
segment_index = int(pipe_geom.num_segments / 2)
start_time = 0 * second()
stop_time = "end"
leakage_mass_rate = 2 * kg() / second()
flow_direction = ["outflow", "perpendicular to the pipe"]

# injection_specific_enthalpy = -20000 * Joule() / kg()  # Constant injection specific enthalpy
sources_sinks["ConstantMassRateSource_2"] = ConstantMassRateSource(pipe_name, pipe_geom, fluid_model, segment_index,
                                                                   start_time, stop_time, flow_direction,
                                                                   leakage_mass_rate, isothermal, verbose=verbose)
...
#%% Time stepping
dt = 2 * second() * np.ones(300)   # 10 minutes
