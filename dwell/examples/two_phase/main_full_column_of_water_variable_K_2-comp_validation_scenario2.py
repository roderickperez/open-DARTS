"""
This validation scenario is reported in the thesis because the reservoir permeability is more logical.

Scenario description:
    Isothermal miscible 2-phase fluid flow with variable K values
    Impure CO2 injection into a wellbore full of water
    ...
    reservoir_cell_perm_list = [200 * milli() * Darcy(), 200 * milli() * Darcy()]
    system_temperature = 25 + 273.15 * Kelvin()
    injection_mass_rate = 2 * kg() / second()

Notes:
    If I use 25 kg/second for the injection rate, OLGA encounters instabilities, so I used 2 kg/second.
    Pay attention that If you change the density calculation method, the initial pressure profile changes as well, and
    so it must be updated in OLGA for validation.
"""
...
# ['vapor', 'aqueous']   vapor is the CO2-rich (either gaseous or liquid CO2 with dissolved water) phase and aqueous
# is the water-rich (water with dissolved CO2) phase
...
initial_fluid_composition = [1e-10, 1 - 1e-10]
initial_fluid_conditions = {'phases_names': ['liquid'], 'phases_compositions': [initial_fluid_composition],
...
# injection_specific_enthalpy = -20000 * Joule() / kg()  # Constant injection specific enthalpy   # For a 1-kilometer well
