...
Notes:
    I used ramp-up time stepping for this scenario because the solution diverges if the initial time steps are not small.
    If the injection rate is 10 kg/s, the changes in phase properties lead to solver failure after some time. Solver
    failure occurs when the pressure of the CO2-rich phase goes higher than the phase saturation pressure. Reducing
    the injection rate to 5 kg/s prevents the solver failure. Increasing the system temperature to a value higher
    than the CO2 critical temperature also prevents the solver failure even at 10 kg/s.
"""
...
pipe_head_pressure = 5 * bar()   # Reduced WHP from 5 to 1 because when the pressure is 5, there will be a small fraction of liquid in the top half of the wellbore as well, and so the initial pressure calculations will be inaccurate, and it leads to illogical high initial velocites in the wellbore..
pipe_head_segment_index = pipe_geom.num_segments - 1   # index starts from zero

initial_fluid_comp_bottom_half = [1e-5, 1e-5, 1 - 2 * 1e-5]
initial_fluid_comp_top_half = [0.01, 0.98, 0.01]
...
initial_C1_mole_fraction = np.concat(([initial_fluid_comp_bottom_half[1]] * int(pipe_geom.num_segments/2), [initial_fluid_comp_top_half[1]] * int(pipe_geom.num_segments/2)))
...
injected_fluid_mole_fractions = [0.98, 0.01, 0.01]
...
#%% Time stepping
# Use ramp-up time steps to relieve convergence problem at the beginning of the simulation probably due to the presence of the initial phase interface in the wellbore
