"""
Production of CO2 from a column of CO2 at a temperature higher than the CO2 critical temperature

Scenario description:
    Isothermal miscible 2-phase fluid flow with variable K values
    Impure CO2 production from a wellbore full of gaseous CO2
    Constant mass rate at the wellhead or constant pressure at the wellhead
    Wellbore is perforated at the lowest segment to the reservoir

Notes:
    I have to use a system temperature higher than the CO2 critical temperature because if liquid CO2 forms, solution diverges.

    Because this is a production scenario, I considered the pressure of the lowermost segment exactly equal to the
    reservoir pressure.

    If a constant production rate is chosen for the wellhead, the following points should be followed to prevent solver
      failure:
    - The production rate should be low enough, otherwise, the pressure of the uppermost segment becomes negative.
    - The initial wellhead pressure should be high enough, otherwise, the pressure of the uppermost segment becomes
      negative.
    - The permeability of the reservoir should be high enough so that it prevents too much pressure drop in the wellbore
      (leads to negative wellhead pressure).

    If a constant wellhead pressure is chosen for the wellhead, the only point that should be considered is that
    the constant wellhead pressure should be lower than the initial wellhead pressure.
"""
...
# enthalpy_eval = define_fluid_model.Enthalpy_PR_PT(components_names)
...
#%% Create pipe model
isothermal = True
system_temperature = 40 + 273.15 * Kelvin()   # at this temperature CO2 saturation pressure is 45 bar
...
initial_fluid_composition = [0.999, 0.001]
initial_fluid_conditions = {'phases_names': ['gas'], 'phases_compositions': [initial_fluid_composition],
                            'pipe_intervals': [[0, pipe_geom.pipe_length]]}   # 0 is the beginning of the pipe and pipe_intervals are TVD
...
initial_CO2_mole_fraction = [initial_fluid_conditions['phases_compositions'][0][0]] * pipe_geom.num_segments
...
# start_time = 0 * second()
# stop_time = "end"
# production_mass_rate = 2 * kg() / second()
#
# flow_direction = ["outflow", "along the pipe"]
#
...
#                                                                  stop_time, flow_direction, production_mass_rate, isothermal, verbose=verbose)
...
start_time = 0 * second()
stop_time = "end"
reservoir_pressure = well_initial_conditions.p_init_segments[segment_index]
reservoir_fluid_mole_fractions = initial_fluid_composition
reservoir_cell_perm_list = [200 * milli() * Darcy(), 200 * milli() * Darcy()]   # [Kx, Ky]
