...
def run_python(self, days=0, restart_dt=0, log_3d_body_path=0):
    ...
    while t < runtime:
        ...
        #     self.reservoir.write_to_vtk(self.output_directory, self.cell_property, self.i_th_step, self.physics)
        # else:
        #     self.reservoir.write_to_vtk_old_discretizer(self.output_directory, self.cell_property, self.i_th_step, self.physics)
        # self.i_th_step += 1
...
#run(discr_type='tpfa', mesh_file='meshes/transfinite.msh')
#run(discr_type='mpfa', mesh_file='meshes/transfinite.msh')
